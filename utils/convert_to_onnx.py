...
def convert_to_onnx(
    ...
    if sample is None:
        batch_pt = torch.rand(
            size=(batch_size, 3, config.dataset.crop_size, config.dataset.crop_size),
    ...
    if batch_size == 1:
        ...
        dynamic_axes = {"input": {0: "batch_size"}, "spoof": {0: "batch_size"}}
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Converting the checkpoint {args.checkpoint} with the config {args.config}"
    )
    ...
    onnx_path = Path(args.checkpoint).parent / Path(
        f"{Path(args.checkpoint).stem}_b1.onnx"
    )
    logger.info("Converting with batch size 1")
    convert_to_onnx(config, model, onnx_path, batch_size=1)

    onnx_path = Path(args.checkpoint).parent / Path(
        f"{Path(args.checkpoint).stem}_dyn.onnx"
    )
